    ...
    SetupFindContactsButton: function(){
        $('#findButton').click(function(){
            ...
            function onContactFindSuccess(contacts){
                $('#resultDisplay').html(contacts[0].name.givenName+" "+contacts[0].name.familyName);
                for (var i = 0; i < contacts[0].emails.lenght; i++){
                    if (i>0){ $('#email').append('<br/>');}
                    $('#email').append(contacts[0].emails[i].value);
                }
                for (var i = 0; i < contacts[0].phoneNumbers.lenght; i++){
                    if(i>0){ $('#tel').append('<br/>'); }
